 ...
 *
 * @author Jashandeep Singh, 1690198282
 * @version 2023-05-07
 */
public class Food implements Comparable<Food> {
	...
	int compValue = name.toLowerCase().compareTo(target.name.toLowerCase());
	if (compValue == 0) {
	    compValue = ORIGINS[origin].toLowerCase().compareTo(ORIGINS[target.origin].toLowerCase());
	...
	String value = String.format("Name:       %s\nOrigin:     %s\nVegetarian: %s\nCalories:   %s", name,
		ORIGINS[origin], isVegetarian, calories);
